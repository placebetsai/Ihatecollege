...
import useSWR from 'swr'; // For auto-refresh on focus/reload
import axios from 'axios';

const fetcher = url => axios.get(url).then(res => res.data);

export default function CivilService() {
  // Live refresh for jobs (USAJobs API — replace 'YOUR_API_KEY' with real one from developer.usajobs.gov)
  const { data: uspsJobs } = useSWR('https://data.usajobs.gov/api/search?Keyword=USPS&ResultsPerPage=3', fetcher, { revalidateOnFocus: true }); // Refreshes on tab focus
  const { data: tsaJobs } = useSWR('https://data.usajobs.gov/api/search?Keyword=TSA&ResultsPerPage=3', fetcher, { revalidateOnFocus: true });
  const { data: fireJobs } = useSWR('https://data.usajobs.gov/api/search?Keyword=Firefighter&ResultsPerPage=3', fetcher, { revalidateOnFocus: true });
  const { data: statePoliceJobs } = useSWR('https://data.usajobs.gov/api/search?Keyword=State+Police&ResultsPerPage=3', fetcher, { revalidateOnFocus: true });
  const { data: dmvJobs } = useSWR('https://data.usajobs.gov/api/search?Keyword=DMV&ResultsPerPage=3', fetcher, { revalidateOnFocus: true });
  const { data: parkRangerJobs } = useSWR('https://data.usajobs.gov/api/search?Keyword=Park+Ranger&ResultsPerPage=3', fetcher, { revalidateOnFocus: true });
  const { data: floridaDmvJobs } = useSWR('https://data.usajobs.gov/api/search?Keyword=Florida+DMV&ResultsPerPage=3', fetcher, { revalidateOnFocus: true });

  // Live refresh for generic images (Unsplash API — random new image each load, no static)
  const { data: uspsImage } = useSWR('https://api.unsplash.com/search/photos?query=USPS+mail+carrier+in+action&per_page=1&orientation=portrait', fetcher, { revalidateOnFocus: true });
  const uspsImgUrl = uspsImage?.results[0]?.urls.regular || ''; // Refreshes to new image

  const { data: tsaImage } = useSWR('https://api.unsplash.com/search/photos?query=TSA+agent+at+airport&per_page=1&orientation=portrait', fetcher, { revalidateOnFocus: true });
  const tsaImgUrl = tsaImage?.results[0]?.urls.regular || '';

  const { data: fireImage } = useSWR('https://api.unsplash.com/search/photos?query=firefighter+in+action&per_page=1&orientation=portrait', fetcher, { revalidateOnFocus: true });
  const fireImgUrl = fireImage?.results[0]?.urls.regular || '';

  const { data: statePoliceImage } = useSWR('https://api.unsplash.com/search/photos?query=state+police+officer&per_page=1&orientation=portrait', fetcher, { revalidateOnFocus: true });
  const statePoliceImgUrl = statePoliceImage?.results[0]?.urls.regular || '';

  const { data: dmvImage } = useSWR('https://api.unsplash.com/search/photos?query=DMV+clerk+at+desk&per_page=1&orientation=portrait', fetcher, { revalidateOnFocus: true });
  const dmvImgUrl = dmvImage?.results[0]?.urls.regular || '';

  const { data: parkRangerImage } = useSWR('https://api.unsplash.com/search/photos?query=state+park+ranger+in+nature&per_page=1&orientation=portrait', fetcher, { revalidateOnFocus: true });
  const parkRangerImgUrl = parkRangerImage?.results[0]?.urls.regular || '';

  const { data: floridaDmvImage } = useSWR('https://api.unsplash.com/search/photos?query=Florida+DMV+clerk&per_page=1&orientation=portrait', fetcher, { revalidateOnFocus: true });
  const floridaDmvImgUrl = floridaDmvImage?.results[0]?.urls.regular || '';
  ...
  return (
    <Layout>
      ...
      <section className="px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto">
          ...
          <div className="rounded-xl p-6 bg-slate-900/80 border border-blue-400/30 shadow-lg shadow-blue-500/20 hover:border-blue-400/70 hover:shadow-xl hover:shadow-blue-500/40 hover:-translate-y-1 transition-all duration-300 ease-in-out animate-[patriotic-glow_6s_infinite_ease-in-out]">
            ...
            <p className="text-gray-400 text-xs mt-4">Refreshed: {uspsJobs?.length || 0} open positions, e.g., {uspsJobs?.[0]?.MatchedObjectDescriptor.PositionTitle || 'Loading...'} ({uspsJobs?.[0]?.MatchedObjectDescriptor.PositionRemuneration[0].MinimumRange || '$'}).</p>
          </div>
          ...
          <div className="rounded-xl p-6 bg-slate-900/80 border border-blue-400/30 shadow-lg shadow-blue-500/20 hover:border-blue-400/70 hover:shadow-xl hover:shadow-blue-500/40 hover:-translate-y-1 transition-all duration-300 ease-in-out animate-[patriotic-glow_6s_infinite_ease-in-out]">
            ...
            <p className="text-gray-400 text-xs mt-4">Refreshed: {tsaJobs?.length || 0} open positions, e.g., {tsaJobs?.[0]?.MatchedObjectDescriptor.PositionTitle || 'Loading...'} ({tsaJobs?.[0]?.MatchedObjectDescriptor.PositionRemuneration[0].MinimumRange || '$'}).</p>
          </div>
          ...
          <div className="rounded-xl p-6 bg-slate-900/80 border border-blue-400/30 shadow-lg shadow-blue-500/20 hover:border-blue-400/70 hover:shadow-xl hover:shadow-blue-500/40 hover:-translate-y-1 transition-all duration-300 ease-in-out animate-[patriotic-glow_6s_infinite_ease-in-out]">
            ...
            <p className="text-gray-400 text-xs mt-4">Refreshed: {fireJobs?.length || 0} open positions, e.g., {fireJobs?.[0]?.MatchedObjectDescriptor.PositionTitle || 'Loading...'} ({fireJobs?.[0]?.MatchedObjectDescriptor.PositionRemuneration[0].MinimumRange || '$'}).</p>
          </div>
        </div>
      </section>
      ...
      <section className="px-4 mt-12">
        ...
        <p className="text-gray-300 text-center mb-8">Every state has entry-level civil service roles with benefits. Check your state's job board.</p>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto">
          {/* State Police */}
          <div className="rounded-xl p-6 bg-slate-900/80 border border-blue-400/30 shadow-lg shadow-blue-500/20 hover:border-blue-400/70 hover:shadow-xl hover:shadow-blue-500/40 hover:-translate-y-1 transition-all duration-300 ease-in-out animate-[patriotic-glow_6s_infinite_ease-in-out]">
            ...
            <p className="text-gray-400 text-xs mt-4">Refreshed: {statePoliceJobs?.length || 0} open positions, e.g., {statePoliceJobs?.[0]?.MatchedObjectDescriptor.PositionTitle || 'Loading...'} ({statePoliceJobs?.[0]?.MatchedObjectDescriptor.PositionRemuneration[0].MinimumRange || '$'}).</p>
          </div>
          ...
          <div className="rounded-xl p-6 bg-slate-900/80 border border-blue-400/30 shadow-lg shadow-blue-500/20 hover:border-blue-400/70 hover:shadow-xl hover:shadow-blue-500/40 hover:-translate-y-1 transition-all duration-300 ease-in-out animate-[patriotic-glow_6s_infinite_ease-in-out]">
            ...
            <p className="text-gray-400 text-xs mt-4">Refreshed: {dmvJobs?.length || 0} open positions, e.g., {dmvJobs?.[0]?.MatchedObjectDescriptor.PositionTitle || 'Loading...'} ({dmvJobs?.[0]?.MatchedObjectDescriptor.PositionRemuneration[0].MinimumRange || '$'}).</p>
          </div>
          ...
          <div className="rounded-xl p-6 bg-slate-900/80 border border-blue-400/30 shadow-lg shadow-blue-500/20 hover:border-blue-400/70 hover:shadow-xl hover:shadow-blue-500/40 hover:-translate-y-1 transition-all duration-300 ease-in-out animate-[patriotic-glow_6s_infinite_ease-in-out]">
            ...
            <p className="text-gray-400 text-xs mt-4">Refreshed: {parkRangerJobs?.length || 0} open positions, e.g., {parkRangerJobs?.[0]?.MatchedObjectDescriptor.PositionTitle || 'Loading...'} ({parkRangerJobs?.[0]?.MatchedObjectDescriptor.PositionRemuneration[0].MinimumRange || '$'}).</p>
          </div>
          ...
          <div className="rounded-xl p-6 bg-slate-900/80 border border-blue-400/30 shadow-lg shadow-blue-500/20 hover:border-blue-400/70 hover:shadow-xl hover:shadow-blue-500/40 hover:-translate-y-1 transition-all duration-300 ease-in-out animate-[patriotic-glow_6s_infinite_ease-in-out]">
            ...
            <p className="text-gray-400 text-xs mt-4">Refreshed: {floridaDmvJobs?.length || 0} open positions, e.g., {floridaDmvJobs?.[0]?.MatchedObjectDescriptor.PositionTitle || 'Loading...'} ({floridaDmvJobs?.[0]?.MatchedObjectDescriptor.PositionRemuneration[0].MinimumRange || '$'}).</p>
          </div>
        </div>
      </section>
